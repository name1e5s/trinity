...
case class Ref(
    local: Systolic#Local,
    direction: Seq[Int],
    fixed: Seq[Boolean],
    its: Seq[Systolic#Iterator],
    maxes: Seq[Int]
) extends Expr {
...
case class IRef(input: Systolic#Input, its: Seq[Systolic#Iterator])
    extends Expr {
  var localRef: Ref = _
}

case class ORef(output: Systolic#Output, its: Seq[Systolic#Iterator])
    extends Expr {
  var localRef: Ref = _
